...
func NewNetworkController(
	...
	log.Printf("creating event broadcaster")
...
func (c *NetworkController) sync(key string) error {
	...
	log.Printf("service network annotation found: %v", annotations)
	...
	for _, pod := range pods {
		...
		for _, status := range networksStatus {
			if isInNetworkSelectionElementsArray(status.Name, networks) {
				log.Printf("processing pod %s/%s: found network %s interface %s with IP addresses %s",
					pod.Namespace, pod.Name, annotations, status.Interface, status.IPs)
				// all IPs of matchiong network are added as endpoints
	...
	if err != nil {
		log.Printf("error updating endpoint: %s", err)
		return err
	}

	log.Printf("endpoint updated succesfully")
	msg := fmt.Sprintf("Updated to use network %s", annotations)

	c.recorder.Event(ep, corev1.EventTypeNormal, msg, "Sync succesful")
	c.recorder.Event(svc, corev1.EventTypeNormal, msg, "Sync succesful")
